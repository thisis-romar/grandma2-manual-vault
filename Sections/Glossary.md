...
section: "Glossary"
slug: "key_glossary"
url: "https://help.malighting.com/grandMA2/en/help/key_glossary.html"
page_count: 0
tags:
  - "type/section"
...
> [!source]- Source
> [MA Lighting Help – Glossary](https://help.malighting.com/grandMA2/en/help/key_glossary.html)
...
**Art-Net:**  
Art-Net 3 Ethernet communication standard. A protocol for transmitting the lighting control protocol DMX 512-A via the User Datagram Protocol of the Internet Protocol suite. For information on how to use Art-Net see [Network Protocols](../Pages/Networking/Network%20DMX%20protocols.md). For information on how to assign Art-Net Presets on ports for the MA Network Switch see [Control the MA Network Switch - Edit Presets](../Pages/Control%20MA%20Network%20Switch/Edit%20presets%20of%20the%20MA%20Network%20Switch.md).
...
**Blind:**  
Create values without DMX output. For more information see [Blind Key](../Keys/Blind%20Key.md) and [Blind Command](../Keywords/Blind%20keyword.md).

**B.O.** (black out):  
Brings dimmer values to zero. For more information see [B.O. Key](../Keys/B.O.%20%28Blackout%29%20Key.md).
...
**Channel:**  
For more information see [Channel Sheet](../Pages/Operate%20Fixtures/Channel%20Sheet.md).

**Channel Sets:**  
Predefined values within the fixture type for fast access.

**Chaser:**  
A chaser is an executor mode that runs randomly, in a loop or bounces in the cue list. For more information see [Chasers](Chasers.md). 

**Cmd** (Command):  
Instructions you enter into the console.

**Command Section:**  
Area on the right of the console used to enter commands into the command line using keys. For more information see [Command section](../Pages/Workspace/Command%20Section.md). 

**Command Line:**  
The command line is located at the bottom of screen 1 and screen 2. It is used to enter commands into the console. For more information see [Workspace - User Interface - Command Line](../Pages/Workspace/Command%20Line.md).

**Constant bit rate (CBR):**  
Constant bit rate defines the bit rate images are encoded with. The data stream is constant. This means that every image has the same size. Constant bit rate is used in [bitmap fixtures](Bitmap%20Fixture.md).

**Control:**  
Control attributes are, for example lamp control, fixture global and scan rate.

**Cue:**  
Cues contain values that were stored. For more information see [What are cues and sequences](../Pages/Command%20Syntax%20and%20Keywords/What%20are%20cues%20and%20sequences.md). 
...
Is a standardized network protocol used on Internet Protocol (IP) networks for dynamically distributing network configuration parameters, such as IP addresses for interfaces and services. With DHCP, computers request IP addresses and networking parameters automatically from a DHCP server, reducing the need for a network administrator or a user to configure these settings manually. To enable or disable DHCP for the MA Network Switch see [Control the MA Network Switch - Enable DHCP client](../Pages/Control%20MA%20Network%20Switch/Enable%20DHCP%20client%20for%20the%20MA%20Network%20Switch.md).
...
Connection for external screen. For more information see [Device Overview](Device%20Overview.md).
...
**Encoder:**  
The five round rotary knobs below screen 1, to control values of attributes or scroll on the screen. For more information see [Device Overview](Device%20Overview.md).

**Ethernet:**  
Is a local network with bus topology, what allows multiply devices access at the same time. It is used for communication between light consoles. The grandMA2 consoles have two Ethernet connectors. For more information see [Device Overview](Device%20Overview.md).

**Executor:**  
Button and fader executor on the console and executor in the virtual playbacks. You store cues on an executor. For more information see [What are executors](../Pages/Executors/What%20are%20executors.md). 
...
**Fixture ID:**  
Unique identifier for each fixture. Set the fixture ID in the [Patch & Fixture Schedule](../Pages/Patching/Adding%20fixtures%20to%20the%20show.md). 
...
**Grand master:**  
Fader rightmost at the console to reduce all dimmer values. For more information see [Device Overview](Device%20Overview.md).

Groups relating to the Groups Pool:

**Groups** relating to the MA Network Switch:  
A logical segment of network users and resources grouped by function, team, or application. This segmentation is without regard to the physical location of the users and resources. Also known as VLAN. To edit groups see [Control the MA Network Switch - Edit Groups](../Pages/Control%20MA%20Network%20Switch/Edit%20groups%20of%20the%20MA%20Network%20Switch.md).
...
Used by network devices for advertising their identity, capabilities, and neighbors on an IEEE 802 local area network. LLDP will be disabled or enabled along with RSTP. To enable or disable RSTP see [Control the MA Network Switch - Configure Ports](../Pages/Control%20MA%20Network%20Switch/Configure%20ports%20of%20the%20MA%20Network%20Switch.md).
...
**Programmer:**  
Is a container with the current selection of fixtures and values of manual adjusted attributes. Programmer values are red values. For more information about the colors see [Workspace – Colors](../Pages/Workspace/Colors.md).
...
Permits intelligent bidirectional communication between devices from multiple manufacturers using a modified DMX512 data link. RDM is an EF 1.0 implementation of ANSI E1.11 (DMX512-A). To use RDM see [RDM](RDM.md).

**Rpm** (revolutions per minute):  
Rotation speed of a gobo wheel in the unit revolutions per minute.

**RSTP** (Rapid Spanning Tree Protocol):  
Provides significantly faster spanning tree convergence after a topology change, introducing new convergence behaviors and bridge port roles to do this. To enable or disable RSTP for a port see [Control the MA Network Switch - Configure Ports](../Pages/Control%20MA%20Network%20Switch/Configure%20ports%20of%20the%20MA%20Network%20Switch.md).

## S

**sACN** (streaming Architecture for Control Networks)  
ANSI E1.31-2009. A protocol to transport DMX512 data over IP networks. To use [sACN see Network Protocols](../Pages/Networking/Network%20DMX%20protocols.md).
...
**Trunk**:  
This is the name given to the interconnection links between switches in a network. The Trunk links are meant to forward the groups based packets. In most case, the Trunk links will be the redundant links. To assign a port the Trunk group see [Control the MA Network Switch - Configure Ports](../Pages/Control%20MA%20Network%20Switch/Configure%20ports%20of%20the%20MA%20Network%20Switch.md).

## U

**UID** (= Unique Identifier):  
All RDM devices have a unique identifier (UID) that consists of a manufacturer ID and serial number. The UID can be used as an option for several RDM keywords. To get a list of all available RDM keywords, see [specific RDM keywords](../Pages/RDM/RDM%20specific%20keywords.md).
...
For more information see [Groups](<#Groups relating to the MA Network Switch>) relating to the MA Network Switch.
...
**XLR A-D:**  
Four pin connector for DMX signal at the back of the console. For more information see [Technical Data](Technical%20Data.md).
