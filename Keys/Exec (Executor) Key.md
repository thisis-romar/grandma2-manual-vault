---
type: "key"
cssclasses:
  - "gma2-key"
key_label: "Exec (Executor) Key"
slug: "key_key_executor"
url: "https://help.malighting.com/grandMA2/en/help/key_key_executor.html"
related_keyword: "[Keywords/Executor](../Keywords/Executor.md)"
tags:
  - "type/key"
summary: "The key Exec is located in the command area on the right of the key Cue."
---

# Exec (Executor) Key

> [!source]- Source
> [MA Lighting Help – Exec (Executor) Key](https://help.malighting.com/grandMA2/en/help/key_key_executor.html)

Version 3.9

# Exec (Executor) Key

The key Exec is located in the command area on the right of the key Cue.

![](../../img/img_exec_key_3_2.png)

_Location key_ _Exec_ 

*   Pressing Exec enters the [Executor keyword](../Keywords/Executor.md) into the command line.

\[Channel\]> Executor

*   Pressing Exec Exec enters the [SpecialMaster keyword](../Keywords/SpecialMaster.md) into the command line.

\[Channel\]> SpecialMaster

Part of [000 Key Index](../000%20Key%20Index.md) · [000 Map of Content](../000%20Map%20of%20Content.md)
