...
prev_page: "[Pages/Networking/Protecting the session and station](Protecting%20the%20session%20and%20station.md)"
next_page: "[Pages/Networking/How to end or leave a session](How%20to%20end%20or%20leave%20a%20session.md)"
depth: 3
tags:
  - "type/page"
  - "section/network"
...
> [!source]- Source
> [MA Lighting Help – Adding devices to the session](https://help.malighting.com/grandMA2/en/help/key_network_session_add_device.html)
...
![](../../img/menu_ma-network-configuration_v3-3.png)

_MA Network Configuration -_ _The Console tab_
...
Consoles and grandMA2 onPCs are called **Stations** and are connected to a session using the [MA Network Control](How%20to%20create%20a%20session.md). They cannot be invited or forced to connect from this interface, but the DMX ports and timecode can be configured. Stations in the session will also appear in the MA Network Configuration.
...
*   **DHCP ETHERCON 1 (ETH0)** - Editing this cell will give a "Yes". This enables DHCP for Ethernet port 1 and will prompt a reboot. Read more about DHCP in the [Using DHCP in MA devices topic](Using%20DHCP%20in%20MA%20devices.md).
...
*   **XLR A** to **XLR F** - These are the female DMX connectors on the back of the console. Editing this cell will open a pop-up that allows you to change the settings. It is described in details in the [Getting DMX in and out of the system](Getting%20DMX%20in%20and%20out%20of%20the%20system.md) topic.
*   **XLR In** - This is the male DMX port on the back of the console. Editing, works just as the female connectors - see above.
*   **MIDI TC** - Choose which Timecode slot the incoming MIDI timecode should be sent to. Please read the [Timecode](../../Sections/Timecode.md) topic for more details about Timecode slots.
*   **SMPTE TC** - Choose which Timecode slot the incoming LTC/SMPTE timecode should be sent to. Please read the [Timecode](../../Sections/Timecode.md) topic for more details about Timecode slots.
...
grandMA2 onPCs are considered "Stations" and can only be added to a session in the [MA Network Control](How%20to%20create%20a%20session.md).
...
When a MA 3D visualizer is added to the table then you will have the same **Session Member** column as the MA NPU and the process is the same. When connecting a MA 3D it can be chosen which **User** it should follow. The users in the show each have a [Stage view](../Patching/Stage%20View.md). The MA 3D is connected to the Stage view, so changing the angle or location of the virtual camera in the Stage view or the MA 3D visualizer, they will both follow along. A MA 3D visualizer will also follow the user into Blind and Preview mode and show the blind and preview output instead of the live output.
...
For details about the MA VPU or any of the things mentioned above, please have a look the [VPU Manual](../../QuickStart/grandMA2%20Quick%20Start%20Guide.md).
...
*   **MA-Net2** - This is the mode needed to have full control and synchronized DMX output from a MA2 system. This is also needed for the onPC nodes to unlock parameters in an onPC system. Please read more in the [Parameter Expansion topic](../System%20Overview/Parameter%20expansion.md).
...
The MA Network Switch has a lot of settings and controlling or configuring it from the console/onPC is all described in the [Control the MA Network Switch](../../Sections/Control%20MA%20Network%20Switch.md) section.

## Bridge

The Bridge is not implemented, so there is nothing to do in this tab.

← [Protecting the session and station](Protecting%20the%20session%20and%20station.md) | [How to end or leave a session](How%20to%20end%20or%20leave%20a%20session.md) →
