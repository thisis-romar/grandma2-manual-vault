...
prev_page: "[Pages/Macros/Conditional expressions](Conditional%20expressions.md)"
depth: 3
tags:
  - "type/page"
  - "section/macro"
...
> [!source]- Source
> [MA Lighting Help – Macro timing](https://help.malighting.com/grandMA2/en/help/key_macro_create_timing.html)
...
Assign Root "DMX\_Protocols"."sACN" /OutActive=on /InActive=on
...
Label Macro "sACN OFF/on" "sACN ON/off"

Go

4

Assign Root "DMX\_Protocols"."sACN" /OutActive=off /InActive=off
...
If the "[Top](../../Keywords/Top.md)" command is sent to the macro then it will run the macro from the first line.

"[GoBack](../../Keywords/GoBack.md)" will execute the previous macro line in a multi line macro. Looking at the example above it will run line 2 if the macro is at line 3 waiting for a Go. If the macro is not active then "GoBack" will run the last macro line.  
  
Running macros can be paused using the "[Pause](../../Keywords/Pause.md)" command. This only makes sense for timed macros with enough time to actually pause it before it is done. You need to do a Go command for the macro to continue.
