...
section_ref: "[Sections/Advanced Fixture Types](../../Sections/Advanced%20Fixture%20Types.md)"
next_page: "[Pages/Advanced Fixture Types/Attribute & Encoder Grouping](Attribute%20&%20Encoder%20Grouping.md)"
depth: 2
tags:
  - "type/page"
  - "section/adv"
...
> [!source]- Source
> [MA Lighting Help – Anatomy of a Fixture Type](https://help.malighting.com/grandMA2/en/help/key_adv_fixture_anatomy.html)
...
The modules contains a list of different **Attributes** that are mapped to DMX channels. The modules are added and defined in the [Module Manager](Module%20Manager.md). The module amount and possible physical relationship between the modules is defined in the [Instance Manager](Instance%20Manager.md).

Inside a module there might be different **Wheels**. A wheel can be color wheels, gobo wheels, animation wheels, or other types of wheels. A color scroller is also often defined as a wheel even though it technically is a roll. All the wheels are defined in the [Wheel Manager](Wheel%20Manager.md).

Fixtures can be build or modified in the grandMA2 software. It is possible to import downloaded fixture types. Please read the [Adding fixtures to the show topic](../Patching/Adding%20fixtures%20to%20the%20show.md) for information about this.
...
![](../../img/menu_fixture-editor_module_v3-3.png)

Module view in the fixture type editor

Here is a view of the first module of the fixture. There are four buttons on the right side of this editor. The top three open the three managers mentioned above. The bottom button on the right side opens the RDM Notification - Read about RDM and creating notifications in the [Configure RDM Notifications topic](../RDM/Configure%20RDM%20notifications.md).

There are some buttons at the bottom of the editor. There are buttons to Add and Delete rows in the different sections in the editor. Edit Row opens the editor for the selected row, it goes "into" the row. Read below for more information about this. The Enable XYZ and Disable XYZ is used to control if the fixture can use [XYZ programming](../../Sections/XYZ.md). Diagnostic opens a pop-up that shows errors or warnings for the selected element. In the lower right corner there can be up to four different arrow buttons. Up and Down arrows will move up and down in the rows. Left and Right arrows will change between different elements. The elements can be modules or when editing a row it can change between different row in the upper level (read more below).
...
*   **Attrib**:  
    ...
    ![](../../img/popup_select-attribute_v3-3.png)
    
    _Select an attribute for the module_
    
      
    This is a long list of all the existing attributes in the show. The list can be edited in the [Attribute & Encoder Grouping](Attribute%20&%20Encoder%20Grouping.md).
*   **Break**:  
...
*   **Stage**:  
    This value is used, if the "Output Stage Value" function is activated in the [Calibrate menu](../Patching/Auto-calibrate%20fixture%20positions.md). This could be values that makes the fixtures open in full output in open white and with a narrow beam.
*   **Snap**:  
    Snap can be turned On. If it is then the attribute will not fade between values - it will jump directly to the new value. When this jump is performed is controlled by the **Snap Percent** value in the cues ([Sequence Executor Sheet](../Command%20Syntax%20and%20Keywords/Looking%20at%20the%20Cue%20Sequence.md)).
*   **Invert**:  
...
*   **Profile**:  
    DMX profiles can be added to an attribute. Editing this opens the Select DMX Profile pop that will list all the existing profiles. Profiles can be added or edited in the Graph Editor - read about it in the [DMX Profiles section](../../Sections/DMX%20Profiles.md).
*   **Mode**:  
...
*   **React to DIM**:  
    ...
    ![](../../img/popup_select-react-to-dim_v3-3.png)
...
**Hint:**  
More modules can be added in the [Module Manager](Module%20Manager.md).
...
![](../../img/menu_fixture-editor_channel_v3-3.png)
...
*   **SubAttrib**:  
    Here the subattribute is defined. Editing this cell will open a pop-up much like the **Select Attribute** shown above, but instead it is **Select Subattribute**. Subattributes are defined in the [Attribute & Encoder Grouping](Attribute%20&%20Encoder%20Grouping.md).
*   **Name**:  
    This is the name that will be displayed as a tap in the calculator pop-up when editing the value of the attribute.
*   **Wheel**:  
    Here it is possible to attach a wheel to the subattribute. Editing this cell will open the **Select Wheel** pop-up that lists the different defined wheels. Wheels can be defined in the [Wheel Manager](Wheel%20Manager.md).
*   **From**:  
...
The channel sets are the basis for information when auto creating presets. The channel sets are shortcuts to easy access to the values. The names are displayed in Fixture Sheets, Channel Sheets, and other places where the attribute values is displayed. The channel sets are also displayed and selectable in the [Smart Window](../Operate%20Fixtures/Using%20The%20Smart%20View.md).

When a row is edited in the Channel Editor, it changes to edit the channel sets. It would look like this:

![](../../img/menu_fixture-editor_channel-set_v3-3.png)
...
*   **Slot No**:  
    If the subattribute has a wheel attached, then the slot number in the wheel can be typed here. Read more about wheels in the [Wheel Manager](Wheel%20Manager.md).
*   **From Phys**:  
...
![](../../img/img_edit-fixture_mode-channel_v3-3.png)
...
![](../../img/img_edit-fixture_mode-channel_edit-row_v3-3.png)
...
*   [Instance manager](Instance%20Manager.md)
*   [Wheel manager](Wheel%20Manager.md)

[Attribute & Encoder Grouping](Attribute%20&%20Encoder%20Grouping.md) →

Part of [Advanced Fixture Types](../../Sections/Advanced%20Fixture%20Types.md) · [000 Map of Content](../../000%20Map%20of%20Content.md)
