...
prev_page: "[Pages/Command Syntax and Keywords/What are cues and sequences](What%20are%20cues%20and%20sequences.md)"
next_page: "[Pages/Command Syntax and Keywords/Storing cues](Storing%20cues.md)"
depth: 2
tags:
  - "type/page"
  - "section/cs"
...
There are several ways to see what cue is running. This topic describes views that do not show the values stored in the cues, but they show the list of cues. To see the content of the cues please read the [Looking at the Cue Content topic](Looking%20at%20the%20cue%20content.md).

## Sequence Executor Sheet

The most common way to look at the Cue Sequence is the Sequence Executor Sheet. It is a window that can be made using the [Create Basic Window pop-up](../../Sections/Windows.md). It could look like this:
...
The sheet can be customized to display only the relevant columns and the columns can be arranged just like you prefer it - this functionality is described in the [Using different view sets in the sheets topic](../Advanced%20Sequence%20Functionality/Using%20different%20view%20sets%20in%20the%20sheets.md).
...
The sheet can be customized and title buttons changed using the Sheet Options - read about them in the [Sheet Options topics](../Operate%20Fixtures/Sheet%20options.md).
...
*   **Trig:**  
    This is the cue trigger. The options here are explained in the [Playing back Cues topic](Playing%20back%20Cues.md).
*   **Trig Time:**  
    This is connected to the trigger. Follow the link above for more information.
*   **Fade:**  
    This is the fade time for all non-snap attributes and dimmer values that does up in value. The [Cue Timings topic](Cue%20timings.md) describes all the different timing columns and their effect on the cue values.
*   **Out Fade:**  
...
*   **Snap Percent:**  
    This can be used to control when "Snap" attributes changes values. It is a percent related to the fade time. This is explained in the [Cue Timings topic](Cue%20timings.md).
*   **MIB:**  
    MIB is "Move In Black". This is introduced in the [What is MIB topic](What%20is%20MIB.md).
*   **Mode:**  
    Cues can have different modes. They are described in the [Using Cue Modes topic](../Advanced%20Sequence%20Functionality/Using%20Cue%20Modes.md).
*   **AE:**  
    AE is "Assert Effects". Read more about effects in the [Effects section](../../Sections/Effects.md).
*   **CMD:**  
    CMD is "Command". Commands (like the ones written the command line input) can be written in the cues. They will then be executed on the Master or Standalone station when the cue is triggered. Have a look at the [Commands in Cues topic](Commands%20in%20cues.md) for more details.
*   **CMD Delay:**  
    This will ad a delay between the triggering of the cue and the execution of the command.
*   **Loop:**  
    Here it is possible to select a cue that should be looped to. Loops are described in details in the [Looping Cues topic](../Advanced%20Sequence%20Functionality/Looping%20cues.md).
*   **Ltime:**  
...
*   **Path:**  
    Here it is possible to select a path for the cue transition. This is described in details in the [Cue Path topic](../Advanced%20Sequence%20Functionality/Cue%20path.md). 
*   **Info:**  
    Here it is possible to add multiple lines of information to a cue. Please have a look at the [Sequence Info View topic](../Advanced%20Sequence%20Functionality/Sequence%20info%20window.md) to learn more.
*   **I.Delay:**  
    This is "Individual Delay". It displays the time for attributes that have individual stored delay times. This is discussed in the [Cue timings topic](Cue%20timings.md).
*   **I.Fade:**  
...
*   **Forced Pos. Mode:**  
    This will open a pop-up that allows you to select a position mode for the cue. Learn more about position modes in the [XYZ and Pan/Tilt in Sequences topic](../XYZ/XYZ%20and%20Pan-Tilt%20in%20cues%20and%20sequences.md).
*   **Preset types Fade / Delay:**  
    Each preset type has two columns called "Delay" and "Fade". These are discussed in the [Cue timings topic](Cue%20timings.md).
...
Starting from the left there is an information "button" that shows the executor number, the sequence number and name, and the cue number and name. It is not actually a button. Next to this is a button that enters or exits the "Edit" mode - This will also change the Encoder Toolbar. Next are six buttons that can be used to control the sequence playback - Please read the [Playing back cues topic](Playing%20back%20Cues.md) for more on this. The last button in the top row is used to change between three different pages for the bottom encoder row.
...
The top part is all the settings that can be changed regarding the sequence. They are all described in the [What are Cues and Sequences topic](What%20are%20cues%20and%20sequences.md).
...
← [What are cues and sequences](What%20are%20cues%20and%20sequences.md) | [Storing cues](Storing%20cues.md) →
