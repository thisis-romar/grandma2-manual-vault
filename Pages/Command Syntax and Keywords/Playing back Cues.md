...
prev_page: "[Pages/Command Syntax and Keywords/Delete Cues](Delete%20Cues.md)"
next_page: "[Pages/Command Syntax and Keywords/Looking at the cue content](Looking%20at%20the%20cue%20content.md)"
depth: 2
tags:
  - "type/page"
  - "section/cs"
...
You can always see the actively running playbacks in the [Running Playbacks window](../Executors/Looking%20at%20the%20active%20executors.md).
...
For details about the executors and the related commands, please read the [Executor section](../../Sections/Executors.md). also have a look at the [Sequence Mini Executor topic](Sequence%20mini%20executor.md).
...
Each cue has an assigned trigger. This can be seen and edited in the **Trig** column in the [Sequence Executor Sheet](Looking%20at%20the%20Cue%20Sequence.md).
...
*   **Timecode:**  
    A cue can have a timecode time as a trigger. The sequence is connected to a Timecode Slot. When the sequence is active and the specified time (in the Trig Time column) is received, then the cue is triggered. Read more about it in the [Record a Timecode Show topic](../Timecode/Record%20a%20Timecode%20Show.md).

**Important**  
When a cue is triggered by timecode, then it always **Asserts**. Read more about assert in the [Using Cue Modes topic](../Advanced%20Sequence%20Functionality/Using%20Cue%20Modes.md).
...
These options can be opened by pressing the Assign key and then one of the executors keys where there is a sequence you want to change. This opens the **Assign Menu**. On the right side of the menu there are some buttons. Please press Options button. This is all the options available for a sequence assigned to an executor. These options are discussed in the [Executor Options topic](../Advanced%20Executor%20Functionality/Executor%20Options.md).
...
← [Delete Cues](Delete%20Cues.md) | [Looking at the cue content](Looking%20at%20the%20cue%20content.md) →
