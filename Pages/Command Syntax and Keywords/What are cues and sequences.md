...
One of the primary ways of doing this is using a **Cue**. Cues contain the stored values. Cues are organized in **Sequences**. A sequence can be assigned to an **Executor** for convenient playback control. Read more about the executors in the [Executors section](key_exec.html).
...
If pools are new to you, then please see the [Pools in General section](key_pools.html).
...
There is a letter indicating the sequences priority. Priorities are described in the [Playing back cues topic](key_cs_playback.html). The different letters are listed in the [Text indicators topic](key_ws_text_indicators.html).

There can be a "T" letter next to priority letter. This shows if tracking is turned On (visible T) or Off for the sequence. Tracking is described in the [What is tracking topic](key_cs_what_is_tracking.html).

There can be two icons in the top row. It can have the Filter icon if the sequence have assigned an input filter. Filters are described beginning from the [What are Filters topic](key_wfm_what_are_filters.html). The other icon is the information icon. This is present if there are any information written for the sequence (it has nothing to do with information in cues - it is about the sequence).
...
The Sequence Pool can also be in "Sheet Mode". This is changed in the Pool Options - read about them in the the [Pools in General section](key_pools.html).
...
*   **Track:**  
    Here you can turn On or Off tracking for the sequence. Tracking is described in the [What is Tracking topic](key_cs_what_is_tracking.html).
*   **Release Firststep:**  
    Here you can turn On or Off "Release First Step". This is related to tracking - follow link above.
*   **CueZero:**  
    Editing this gives you the pop-up used to select the Cue Zero mode. Read more about Cue Zero in the [Cue Zero topic](key_adv_seq_cue_zero.html).
*   **CueZero Extract:**  
    The Cue Zero Extract can be turned On or Off. It is related to the Cue Zero function - follow link above.
*   **Input Filter:**  
    Here you can set an input filter. Filters and sequences are described in the [Apply worlds and filters to executors and sequences topic](key_wfm_apply_to_exec_seq.html).
*   **Timecode Slot:**  
    Here you can select a timecode slot for the sequence. Read the [Timecode section](key_timecode.html) to learn more about using timecode with sequences.
*   **Info:**  
    Here it is possible to add multiple lines of information to the sequence.
*   **Forces Pos. mode:**  
    The entire sequence can have a forced position mode. This is relevant when working with a mix of Pan/Tilt values and XYZ values. Read more about this in the [XYZ and Pan/Tilt in Sequences topic](key_xyz_pan_tilt_in_cues_sequences.html).
*   **Used Count:**  
